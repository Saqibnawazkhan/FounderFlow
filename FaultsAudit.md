...
- [~] **P0-4 · [BUG] Money stored as `Float` (Prisma), not `Decimal`.** UNBLOCKED but still deferred — Tier 2 env separation landed 2026-07-02, so the schema change can now be exercised safely against local Postgres + staging Supabase before it hits prod. The 30-40-file cascade (`.toFixed`, `.toLocaleString`, `zod.number()`, RSC serialization) is still real; ship as a dedicated commit rather than folding into a batch. Real-world drift risk stays bounded — JS number sums are exact up to ~9 quadrillion.
...
- [~] **F1 · 🔴 [BUG] `Float` money type + client-side sums** — unblocked with P0-4 (Tier 2 env separation landed 2026-07-02); still tracked as a dedicated schema-change commit rather than a batch cleanup.
