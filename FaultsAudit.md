...
- [~] **T3 · 🟠 [FEAT] No kanban drag-reorder, no bulk edit, no bulk delete.** ✔ Bulk edit + bulk delete shipped 2026-07-04. List view now has per-row + select-all checkboxes and a floating action bar: bulk "Move to <status>" + bulk Delete. Server actions `bulkUpdateTaskStatusAction` / `bulkDeleteTasksAction` push the single-task permission rule into a scoped `updateMany`/`deleteMany` (admin = any company task; others = own assigned/created only; delete = creator/admin only), emit ONE summary activity row (not one per task), sweep task-deep-link notifications, and fire the bulk-mutation canary. Skipped-for-permission rows are reported honestly in the toast. **Still open:** kanban drag-REORDER within a column (needs a schema `order` field) — tracked separately; status-change-by-drag across columns already works.
