...
**Status: all 26 walked. 21 fixed across 3 commits (bc6dd75 + a0945eb + this batch). 5 skipped (3 false positives, 2 cost-vs-benefit) — marked `[~]` with reasons inline.**
...
- [x] **H1** — `lib/actions/transactions.ts` (deleteTransactionAction activity log) — fixed bc6dd75
...
- [x] **H2** — `lib/actions/tasks.ts` (deleteTaskAction activity log) — fixed bc6dd75
...
- [x] **H3** — `auth.config.ts` (middleware redirect for members) — fixed bc6dd75
...
- [x] **H4** — `app/(app)/projects/[id]/edit-project-modal.tsx` (`defaultValues.targetEndDate`) — fixed bc6dd75
...
- [x] **H5** — `components/layout/sidebar.tsx` (currentUser hydration window) — fixed bc6dd75
...
- [x] **H6** — `lib/auth.ts` `authorize()` (fire-and-forget lastSignInAt) — fixed bc6dd75
...
- [x] **H7** — `router.refresh()` not wrapped in `startTransition` (3 real sites + 2 false positives) — fixed bc6dd75
...
- [x] **M1** — `lib/actions/budgets.ts` `updateBudgetAction` — fixed a0945eb
...
- [x] **M2** — `lib/actions/notifications.ts` `markAllNotificationsReadAction` — fixed a0945eb
...
- [x] **M3** — `lib/actions/notifications.ts` `clearNotificationsAction` — fixed a0945eb
  Same shape — `deleteMany` returns `count`, we discard it.
  **Fix:** same pattern as M2.

- [x] **M4** — `lib/email/send.ts` (SMTP error path) — fixed a0945eb
...
- [x] **M5** — `lib/actions/team.ts` (inviteUserAction email failure) — UI fallback was already correct (line 298 shows the URL when emailSent=false); M4's SMTP-to-Sentry covers the alerting half (a0945eb)
...
- [x] **M6** — `lib/auth.ts` `authorize()` (failed password silent) — fixed a0945eb (captures userId + hashPrefix only — no email to avoid PII / enumeration in Sentry)
...
- [x] **M7** — `components/layout/topbar.tsx` `handleLogout` (logoutAction result discarded) — fixed a0945eb (logoutAction now returns ActionResult; topbar checks before clearing Zustand)
...
- [x] **M8** — `app/(app)/settings/settings-client.tsx` `handleLogout` — fixed a0945eb (same fix as M7)
  Same bug as M7.
  **Fix:** same fix.

- [~] **M9** — `app/(app)/team/team-client.tsx` (role change refresh missing) — SKIPPED: false positive. The success branch at team-client.tsx:50 already calls `refresh()`.
...
- [x] **M10** — `app/api/cron/sweep-time-entries/route.ts` (error context missing) — fixed a0945eb (per-entry try/catch, 206 status on partial)
...
- [x] **M11** — `app/api/cron/materialize-recurring/route.ts` (Sentry capture context) — fixed a0945eb (companyId + counts in extra, 206 status on partial)
...
- [x] **L1** — `components/time/clock-widget.tsx` (visibility-aware ticker) — fixed this batch (setNow on visibilitychange so the running pill repaints immediately on tab refocus)
...
- [~] **L2** — `components/comments/comment-thread.tsx` `refresh()` — SKIPPED: flicker is bounded by startTransition; making onChanged awaitable would propagate Promise<void> through multiple callers for a marginal UX win.
...
- [~] **L3** — `components/layout/sidebar.tsx` (interval cleanup race) — SKIPPED: false positive. `if (!cancelled && res.success)` already gates the setState after await; no warning fires.
...
- [~] **L4** — `lib/rate-limit.ts` (in-memory bucket recycle) — SKIPPED: module doc already calls out the caveat prominently; adding a Sentry breadcrumb on init would spam (every cold start).
...
- [x] **L5** — `lib/sentry-server.ts` (auto-context) — fixed this batch (`userId` + `companyId` are first-class params now; auto-populate Sentry user + tags)
...
- [x] **L6** — `next.config.js` (Sentry config completeness) — fixed this batch (throws at build if SENTRY_AUTH_TOKEN / ORG / PROJECT are partially set)
...
- [~] **L7** — `app/api/cron/materialize-recurring/route.ts` (partial-failure HTTP status) — SKIPPED: covered by M11 (same 206 treatment applied in that fix; redundant entry).
...
- [x] **L8** — `lib/actions/comments.ts` `createCommentAction` (notification fan-out) — fixed this batch (returns `notifiedCount` from the actual createMany; UI toast distinguishes pinged vs attempted-but-failed)
